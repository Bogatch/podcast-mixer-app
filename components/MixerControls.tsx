        ...
        <div>
            ...
            <div className="relative">
                <textarea
                    ...
                    readOnly
                    value={value}
                    className="w-full bg-gray-700 border border-gray-600 rounded-md p-2 pr-10 text-sm text-gray-200 resize-none overflow-hidden"
                    rows={1}
                />
                <button
                    onClick={handleCopy}
...
export const MixerControls: React.FC<MixerControlsProps> = ({
  ...
  return (
    <div className="bg-gray-800 p-6 rounded-lg border border-gray-700 shadow-lg space-y-6">
      <div>
        <h3 className="text-lg font-semibold mb-4 text-white">{t('mixer_title')}</h3>
        
        <ControlWrapper isEnabled={hasTracks}>
          <div className="mb-6">
            <div className="flex items-center space-x-2">
              <label htmlFor="mix-duration" className="block text-sm font-medium text-gray-300">
                {t('mixer_crossfade')}
              </label>
            ...
            <div className="flex items-center space-x-4 mt-2">
              ...
              />
              <span className="w-24 bg-gray-700 text-teal-300 font-mono text-sm sm:text-base text-center py-1 rounded-md border border-gray-600">
        ...
        <ControlWrapper isEnabled={showDuckingControl}>
            <div className="mb-6">
              <div className="flex items-center space-x-2">
                <label htmlFor="ducking-amount" className="block text-sm font-medium text-gray-300">{t('mixer_ducking')}</label>
            ...
            <div className="mb-6">
              <div className="flex items-center space-x-2">
                <label htmlFor="ramp-up-duration" className="block text-sm font-medium text-gray-300">{t('mixer_ramp_up')}</label>
        ...
        <ControlWrapper isEnabled={showUnderlayControl}>
           <div className="mb-6">
            <div className="flex items-center space-x-2">
              <label htmlFor="underlay-volume" className="block text-sm font-medium text-gray-300">{t('mixer_underlay_volume')}</label>
      ...
      <div className="border-t border-gray-700 pt-6 space-y-6">
         <div >
            <div className="flex items-center space-x-2 mb-4">
              <h3 className="text-lg font-semibold text-white flex items-center"><MagicWandIcon className="w-5 h-5 mr-2 text-teal-400" />{t('ai_title')}</h3>
              <InfoTooltip text={t('tooltip_ai')} position="right" />
            </div>
            <div className="flex items-center justify-between mb-4">
              <label htmlFor="auto-trim-enable" className="text-sm font-medium text-gray-300">{t('ai_trim')}</label>
              <button id="auto-trim-enable" onClick={() => onTrimSilenceChange(!trimSilenceEnabled)}
                  className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-teal-500 ${trimSilenceEnabled ? 'bg-teal-500' : 'bg-gray-600'}`} disabled={isMixing}>
            ...
            <ControlWrapper isEnabled={trimSilenceEnabled}>
              <div>
                  <div className="flex items-center space-x-2"><label htmlFor="auto-trim-threshold" className="block text-sm font-medium text-gray-300">{t('ai_threshold')}</label><InfoTooltip text={t('tooltip_ai_threshold')} position="right" /></div>
                  <div className="flex items-center space-x-4 mt-2">
             ...
             <div className="flex items-center justify-between mt-4">
              <label htmlFor="normalize-tracks" className="text-sm font-medium text-gray-300">{t('mixer_normalize_tracks')}</label>
              ...
              <button id="normalize-tracks" onClick={() => onNormalizeTracksChange(!normalizeTracks)}
                  className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-teal-500 ${normalizeTracks ? 'bg-teal-500' : 'bg-gray-600'}`} disabled={isMixing}>
        ...
        {hasTracks && (
            <div className="space-y-4">
                <div className="flex items-center space-x-2"><h3 className="text-lg font-semibold text-white flex items-center"><LightBulbIcon className="w-5 h-5 mr-2 text-teal-400" />{t('ai_content_assistant_title')}</h3><InfoTooltip text={t('tooltip_ai_content')} position="right" /></div>
                {!isPro ? (<button onClick={onOpenUnlockModal} className="w-full flex items-center justify-center px-4 py-2 bg-teal-900 hover:bg-teal-800 border border-teal-700 text-teal-200 font-semibold rounded-md transition-colors"><SparklesIcon className="w-5 h-5 mr-2" /><span>{t('header_get_pro')}</span></button>) : (<>
                        <button onClick={onSuggestContent} disabled={!canSuggest} className="w-full flex items-center justify-center px-4 py-2 bg-teal-500 hover:bg-teal-600 disabled:bg-gray-500 text-white font-semibold rounded-md transition-colors duration-200 disabled:cursor-not-allowed">
        ...
        <div>
            <h3 className="text-lg font-semibold mb-2 text-white">{t('output_title')}</h3>
            <div className="bg-gray-700/50 p-4 rounded-md text-center mb-4 space-y-2 border border-gray-600">
              <div>
              ...
              <div className="flex items-center justify-center pt-2 space-x-4">
                  <div className="flex items-center space-x-2"><label htmlFor="normalize-output" className="text-sm font-medium text-gray-300 flex items-center"><SpeakerWaveIcon className="w-4 h-4 mr-2 text-teal-400" />{t('output_normalize')}</label><InfoTooltip text={t('tooltip_normalize')} position="top" /></div>
                  <button id="normalize-output" onClick={() => onNormalizeOutputChange(!normalizeOutput)}
                      className={`relative inline-flex flex-shrink-0 h-6 w-11 border-2 border-transparent rounded-full cursor-pointer transition-colors ease-in-out duration-200 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-700 focus:ring-teal-500 ${normalizeOutput ? 'bg-teal-500' : 'bg-gray-600'}`} disabled={isMixing}>
            ...
            <div className="space-y-3">
              ...
              {mixedAudioUrl && (
                <div className="pt-2">
                    <div className="mb-3"><p className="text-sm font-semibold text-center text-gray-400 mb-2">{t('output_preview')}</p><audio controls src={mixedAudioUrl} className="w-full">{t('output_audio_not_supported')}</audio></div>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        <button onClick={isPro ? onExportAudio : onOpenUnlockModal} disabled={!canAttemptExport} className="w-full flex items-center justify-center px-4 py-3 bg-green-500 hover:bg-green-600 disabled:bg-gray-500 text-white font-semibold rounded-md transition-colors duration-200 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-green-500">{!isPro && <SparklesIcon className="w-5 h-5 mr-2 text-yellow-400" />}<DownloadIcon className="w-5 h-5 mr-2" />{t('output_export_audio')}</button>
                        <button onClick={isPro ? onExportProject : onOpenUnlockModal} disabled={!canAttemptExport} className="w-full flex items-center justify-center px-4 py-3 bg-indigo-500 hover:bg-indigo-600 disabled:bg-gray-500 text-white font-semibold rounded-md transition-colors duration-200 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-indigo-500">{!isPro && <SparklesIcon className="w-5 h-5 mr-2 text-yellow-400" />}<ArchiveBoxIcon className="w-5 h-5 mr-2" /><span>{t('output_export_project')}</span></button>
                    </div>
                </div>
              )}
               <button onClick={onSaveProject} disabled={isSaving || isMixing} className="w-full flex items-center justify-center px-4 py-3 bg-gray-600 hover:bg-gray-500 disabled:bg-gray-500 text-white font-semibold rounded-md transition-colors duration-200 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-gray-500">
